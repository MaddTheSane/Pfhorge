...
+ (NSMutableArray *)convertMarathonDataToArchived:(NSData *)theData levelNames:(NSMutableArray *)theLevelNamesEXP error:(NSError**)outError
{
    ...
    for (i = 1; i <= numberOfLevels; i++) {
        ...
        [progress setStatusText:[NSString localizedStringWithFormat:NSLocalizedString(@"Converting \"%@\"...", @"Converting “%@”…"), [theLevelNames objectAtIndex:(i - 1)], nil]];
...
- (void)saveAmbientSoundsForLevel:(LELevelData *)level
{
    ...
    for (PhAmbientSound *currentObj in theObjects) {
        [self saveUnsignedShort:[currentObj flags]];
...
{
    return;
    NSLog(@"%@ : %u", theMessage, [self getUnsignedShort]);
    theCursor -= 2;
}
